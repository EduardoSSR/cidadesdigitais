(function () {
    ...
    function InjecaoInfo($http, $routeParams) {
        ...
        /*===================== LOTES  ====================*/
        vm.getLotes = getLotes;
        vm.putLote = putLote;
        vm.getLoteById = getLoteById;
        vm.getLoteItens = getLoteItens;
        vm.putLoteItens = putLoteItens;
        ...
        vm.putEntidade = putEntidade;
        vm.deleteCd = deleteCd;
        vm.postEntidade = postEntidade;
        vm.deleteEntidade = deleteEntidade;
        ...
        function getEmpenhoItensById(empenhoId) {
            return $http.get('read/empenhoItens/' + empenhoId)
        }
        ...
        function putLote(lote) {
            return $http.put('read/lotes', lote);
        }

        function getLoteById(loteId) {
            return $http.get("read/lotes/" + loteId);
        }

        function getLoteItens(loteId) {
            return $http.get("read/loteItens/" + loteId);
        }

        function putLoteItens(itensLote) {
            return $http.put('read/loteItens/', itensLote);
        }
        ...
        function deleteEntidade(cnpj) {
            return $http.delete('read/entidades/' + cnpj);
        }
        ...
        function getFaturaEdit(faturaId) {
            return $http.get('read/fatura/' + faturaId);
        }
        ...
        function getItensFatura(faturaId) {
            return $http.get('/read/faturaItens/' + faturaId);
        }
        ...
        function alterItemFatura(faturaEmpenho) {
            return $http.put('read/faturaItens', faturaEmpenho);
        }


        function delItemFatura(itemFatura) {
            return $http.delete('read/faturaItens/' + itemFatura.fatura_num_nf + '/' + itemFatura.cod_empenho + '/' + itemFatura.cod_item + '/' + itemFatura.cod_tipo_item);
        }
        ...
        function putEntidade(entidade) {
            return $http.put('read/entidades', entidade);
        }
        ...
        function removerProcesso(codProcesso, codIbge) {
            console.log(codProcesso, codIbge)
        ...
        function getEditUacom(uacom) {
            ...
            for (var x = 0; x < acompanhamento.length - 1; x++) {
                if (acompanhamento[x + 1].cd_municipio_cod_ibge == acompanhamentoCd[acompanhamentoCd.length - 1].cd_municipio_cod_ibge && acompanhamento[x + 1].data == acompanhamentoCd[acompanhamentoCd.length - 1].data) {
                    ...
                    acompanhamentoCd[acompanhamentoCd.length] = acompanhamento[x + 1];
                }
            }

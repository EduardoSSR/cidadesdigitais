angular.module('cidadesdigitais').controller('admCategoriaController', function ($scope,InjecaoInfo,growl,$stateParams,$http, $window) {
  ...
  $scope.getCategoriaById = function(){
    InjecaoInfo.getCategoriaById($stateParams.idCategoria)
        .success(function (categoria) {
            $scope.categorias = categoria;
...
$scope.removerCategoria = function(categoria){
 ...
 /*if(window.location.href == "http://172.25.117.3:3000/categoria"){
    $scope.getCategoria();
 }else if(window.location.href == "http://172.25.117.3:3000/editCategoria/" + $stateParams.idCategoria){

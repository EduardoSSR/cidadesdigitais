angular.module('cidadesdigitais').controller('editUsuarioController', function ($scope, $http, $stateParams, growl, $window, InjecaoInfo) {
    ...
    if (permitido) {
        ...
        if($stateParams.usuarioId == $window.sessionStorage.idUser){
            carregaController();
        }else{
     ...
     function carregaController(){
        ...
        if ($stateParams.usuarioId) {
            $http.get('read/usuario/' + $stateParams.usuarioId).success(function (usuario) {
        ...
        $scope.submeter = function (usuario) {
            if (!$scope.formularioAddUsuario.$pristine) {
                ...
                if ($scope.formularioAddUsuario.$valid) {
                    if ($scope.usuario.cod_usuario == $stateParams.usuarioId) {
